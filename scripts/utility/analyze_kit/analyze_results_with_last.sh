...
function get_summary_data(){
  ...
  pass_num=`grep -rn "<actual_result>PASS" ${result_dir} | wc -l`
  fail_num=`grep -rn "<actual_result>FAIL" ${result_dir} | wc -l`
...
function get_data(){
  ...
  pass_num=`grep -rn "<actual_result>PASS" $result_save_path/$1 | wc -l`
  fail_num=`grep -rn "<actual_result>FAIL" $result_save_path/$1 | wc -l`
  let block_num=total_tcs-pass_num-fail_num
  let total_run=total_tcs-block_num
  run_rate_percent=$(get_percent $total_run $total_tcs)%
  pass_rate_percent=$(get_percent $pass_num $total_tcs)%
...
do
  ...
  do
    ...
    total=`grep 'execution_type="auto"' $res_file | wc -l`
    passed=`grep "<actual_result>PASS" $res_file | wc -l`
    failed=`grep "<actual_result>FAIL" $res_file | wc -l`
    let blocked=total-passed-failed
