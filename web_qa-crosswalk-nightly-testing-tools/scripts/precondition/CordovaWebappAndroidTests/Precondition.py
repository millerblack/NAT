...
class Precondition:
    ...
    def set_precondition(self):
        ...
        downloaded_helloworld_file = "%s/helloworld.apk" % save_dir

        if not os.path.exists(apk_resource_dir):
            os.mkdir(apk_resource_dir)

        if not os.path.exists(downloaded_helloworld_file):
            s, o = commands.getstatusoutput("wget %s/helloworld.apk -P %s" % (package_release_url, save_dir))
            if s == 0:
                 shutil.copy(downloaded_helloworld_file, apk_resource_dir)
            else:
                print "Error [%s] happened when testing '%s' 'cordova-webapp-android-tests' on '%s'." % (o, self.mode, self.device_name)
        else:
            shutil.copy(downloaded_helloworld_file, apk_resource_dir)
