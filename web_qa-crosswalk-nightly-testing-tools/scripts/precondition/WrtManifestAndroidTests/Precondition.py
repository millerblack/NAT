...
class Precondition:
    ...
    def set_precondition(self):
        ...
        unzipped_test_dir = "%s/opt/wrt-manifest-android-tests" % unzipped_dir
        apk_dir = "%s/../repo/%s/%s/%s/%s/testsuites-%s/%s/apks-manifest" % (middle_tmp_dir, crosswalk_type, test_platform, self.binary_branch, self.binary_version, self.mode, self.device_arch)
